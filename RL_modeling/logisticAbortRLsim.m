...
addRequired(p,'logisticParams',@isstruct)
...
switch forgettingType
    ...
    case 'decayToInitialValues'
        if (length(forgettingParams) ~= 1)
            error(['With forgettingType = decayToInitialValues, there should be 1 parameter but ' num2str(length(forgettingParams)) ' were given'])
        else
