function [numSR,numLR,nS,nL,RoEoptimalities,EoRoptimalities,nSaborted,nLaborted,...
    numAborted,percentCompletedPR] = driftRLsim_valueUpdate(sessionType,nTrials,...
    agentType,nAgents,actionSelectionMethod,agentParams,driftParams,driftType,varargin)
        ...
        if (action == 1) % if chose action 1 (SR side)
            switch p.Results.driftType
                case 'value_based_drift'
                    [trajectory,hitBound,abortInd] = drift_valueUpdateProcess(Q(1),Q(2),driftParams.drift_rate,Ps,driftParams.noise_amplitude,0,10,Ps);
                case 'value_based_start'
                    [trajectory,hitBound,abortInd] = drift_valueUpdateProcess(Q(1),Q(2),driftParams.drift_rate,Ps,driftParams.noise_amplitude,-(Q(1)-Q(2)),10,Ps);
            end
            if (~hitBound)
                if (p.Results.fullANS)
            ...
            switch p.Results.driftType
                case 'value_based_drift'
                    [trajectory,hitBound,abortInd] = drift_valueUpdateProcess(Q(1),Q(2),driftParams.drift_rate,Pl,driftParams.noise_amplitude,0,10,Pl);
                case 'value_based_start'
                    [trajectory,hitBound,abortInd] = drift_valueUpdateProcess(Q(1),Q(2),driftParams.drift_rate,Pl,driftParams.noise_amplitude,-(Q(2)-Q(1)),10,Pl);
            end
            if (~hitBound)
                if (p.Results.fullANS)
