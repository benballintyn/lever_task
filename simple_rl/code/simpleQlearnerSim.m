...
if (exist('epsilons','var')) % if using epsilon greedy method
    performance = zeros(length(alphas),length(gammas),length(epsilons),4); % Fraction of max RoE values
    ...
    performance = zeros(length(alphas),length(gammas),length(temps),4);
                ...
                for i=1:nAgents
                    ...
                    for t=1:1000
                        ...
                        if (action == 1) % if chose action 1 (SR side)
                            ...
                            nL{a,g,e,s}(t) = nL{a,g,e,s}(t) + 1;
                        end
                        Q(action) = Q(action) + alphas(a)*(r + gammas(g)*max(Q) - Q(action)); % update the q-table
                    end
                    curRoE_rstar = RoE_rstar(nPresses,RoE_NL_optimal(s),SR,LR,Ps);
                    optimality = sum(rewards)/curRoE_rstar;
                    optimalities(i) = optimality;
                    numSR{a,g,e,s} = [numSR{a,g,e,s} sum(actions == 1)];
                    numLR{a,g,e,s} = [numLR{a,g,e,s} sum(actions == 2)];
                end
                pS{a,g,e,s} = nS{a,g,e,s}./(nS{a,g,e,s} + nL{a,g,e,s});
                pL{a,g,e,s} = nL{a,g,e,s}./(nS{a,g,e,s} + nL{a,g,e,s});
                performance(a,g,e,s) = mean(optimalities);
            end
            if (use_e_greedy)
                disp(['alpha = ' num2str(alphas(a)) ' gamma = ' num2str(gammas(g)) ' epsilon = ' num2str(epsilons(e)) ' performance = ' num2str(performance(a,g,e,:))])
            else
                disp(['alpha = ' num2str(alphas(a)) ' gamma = ' num2str(gammas(g)) ' temp = ' num2str(temps(e)) ' performance = ' num2str(performance(a,g,e,:))])
            end
        end
    end
end
savedir = '~/phd/lever_task/simple_rl/utility_func1_softmax_largeSweep';
